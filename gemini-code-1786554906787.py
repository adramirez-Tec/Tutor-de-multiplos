...
# Sidebar - Configuración e Instrucción del Gem
with st.sidebar:
    st.header("⚙️ Configuración del Gem")
    
    if api_key:
        st.success("✅ API Key configurada internamente", icon="🔒")
    else:
...
if st.button("🚀 Generar Informe de Valuación con IA", type="primary", use_container_width=True):
    if not api_key:
        st.error("⚠️ No hay una API Key configurada en los Secrets ni ingresada manualmente.")
    else:
